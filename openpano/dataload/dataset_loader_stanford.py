...
class Dataset(torch.utils.data.Dataset):
    ...
    def readRGBPano(self, path):
        ...
        rgb = cv2.imread(path)
        if rgb is None:
            rgb = cv2.imdecode(np.fromfile(path, dtype=np.uint8), flags=cv2.IMREAD_COLOR)
            # rgb = np.zeros((self.pano_h, self.pano_w, 3), dtype=np.uint8)
    ...
    def readDepthPano(self, path):
        ...
        if depth is None:
            depth = cv2.imdecode(np.fromfile(path, dtype=np.uint8), flags=cv2.IMREAD_COLOR)
