...
VRPE_LUT.append(VRPE(1,128,256,1))
HRPE_LUT.append(HRPE(1,128,256,1))

VRPE_LUT.append(VRPE(2,64,128,1))
HRPE_LUT.append(HRPE(2,64,128,1))

VRPE_LUT.append(VRPE(4,32,64,1))
HRPE_LUT.append(HRPE(4,32,64,1))

VRPE_LUT.append(VRPE(8,16,32,1))
HRPE_LUT.append(HRPE(8,16,32,1))

VRPE_LUT.append(VRPE(16,8,16,1))
HRPE_LUT.append(HRPE(16,8,16,1))
...
class EGTransformer(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        split_size = model_cfg['split_size']
        ...
        hybrid_backbone = model_cfg['hybrid_backbone']
        use_chk = model_cfg['use_chk']
        ...
        se_layer = False
        depths = depth
        # img_size_pano=256; in_chans=3; embed_dim=32;
        ...
        self.ref_point256x512 = genSamplingPattern(128, 256, 3, 3).cuda()
        self.ref_point128x256 = genSamplingPattern(64, 128, 3, 3).cuda()
        self.ref_point64x128 = genSamplingPattern(32, 64, 3, 3).cuda()
        self.ref_point32x64 = genSamplingPattern(16, 32, 3, 3).cuda()
        self.ref_point16x32 = genSamplingPattern(8, 16, 3, 3).cuda()
...
@register_model
def EGDepthModel(split_size=[1,1,1,1,1,1,1,1,1], img_size=[512, 1024], num_heads=[1,2,4,8,8,4,2,1,16]):
    model = EGTransformer(patch_size=4, embed_dim=32, depth=[2,2,2,2,2,2,2,2,2],
        split_size=split_size, num_heads=num_heads, mlp_ratio=4., img_size=img_size)
    return model
