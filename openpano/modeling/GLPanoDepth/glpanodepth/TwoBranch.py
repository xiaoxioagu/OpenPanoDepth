...
class TwoBranch(nn.Module):
    ...
    def __init__(self,model_cfg):
        ...
        self.cube_h = image_height//2
        self.RGB_branch = RGB_module()
        self.depth_branch = Depth_module()
        self.to_tensor = transforms.ToTensor()
        self.normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        self.e2c = Equirec2Cube(image_height, image_width, image_height//2)

    def forward(self, img):
        cubemap = np.zeros((img.shape[0], self.cube_h, self.cube_h * 6, 3))
        self.cubemap = torch.zeros((img.shape[0],3, self.cube_h, self.cube_h * 6))
        self.img = torch.zeros((img.shape[0], 3, self.image_height, self.image_width))
        img = img.cpu().numpy()
        bz, _, h, w = img.shape
        img = img.transpose(0,2,3,1)
        for i in range(bz):
            cubemap[i] = self.e2c.run(img[i])
            x = self.normalize(self.to_tensor(cubemap[i]))
            y = self.normalize(self.to_tensor(img[i]))
            self.cubemap[i] = x
            self.img[i] = y

        self.cubemap = self.cubemap.cuda()
        self.img = self.img.cuda()

        inputs = {}
        inputs["cube_rgb"] = torch.stack(torch.split(self.cubemap, self.cube_h , -1),dim = 1)

        trans_depth, f1, f2, f3, f4 = self.Transformer_branch(inputs["cube_rgb"])
        rgb1, rgb2, rgb3, rgb4 = self.RGB_branch(self.img)
