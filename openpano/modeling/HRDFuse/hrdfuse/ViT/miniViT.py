...
class mViT(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm == 'linear':
            ...
            return regression_head, torch.softmax(y, dim=1), range_attention_maps, queries, x
        else:
            y = torch.sigmoid(y)
        y = y / y.sum(dim=1, keepdim=True)
        return regression_head, y, range_attention_maps, queries, x


class tangent_ViT(nn.Module):
    def __init__(self, in_channels, n_query_channels=128, patch_size=16, dim_out=256,
                 embedding_dim=128, num_heads=4, norm='linear'):
        super(tangent_ViT, self).__init__()
        self.norm = norm
        self.n_query_channels = n_query_channels
        self.patch_transformer = PatchTransformerEncoder(in_channels, patch_size, embedding_dim, num_heads)
        self.dot_product_layer = PixelWiseDotProduct()
    ...
    def forward(self, x):
        ...
        if self.norm == 'linear':
            ...
            return torch.softmax(y, dim=1), queries
        else:
            y = torch.sigmoid(y)
        y = y / y.sum(dim=1, keepdim=True)
        return y, queries
