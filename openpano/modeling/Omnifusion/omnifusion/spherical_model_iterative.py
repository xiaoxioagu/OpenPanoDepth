...
class spherical_fusion(nn.Module):
    def __init__(self, base_cfg):
        self.nrows = base_cfg["nrows"]
        self.npatches = base_cfg["npatches_dict"][self.nrows]
        self.patch_size = eval(base_cfg["patch_size"])
        self.fov = base_cfg["fov"]
        self.iter = base_cfg["iter"]
        ...
        self.down1 = nn.Conv3d(512, 32, kernel_size=1, stride=1, padding=0)
        #self.down2 = nn.Conv3d(512, 512//64, kernel_size=1, stride=1, padding=0)
        self.transformer = Transformer_cascade(512, self.npatches, depth=6, num_heads=4)
    ...
    def forward(self, high_res, confidence=False):
        ...
        for i in range(self.iter-1):
            ...
            high_xyz = high_xyz * high_res_patch_depth
            high_xyz = high_xyz.permute(0, 4, 1, 2, 3).reshape(bs*n_patch, 3, patch_h//4, patch_w//4)
            point_feat = self.mlp_points2(high_xyz)
            point_feat = point_feat.reshape(bs, n_patch, -1, patch_h//4, patch_w//4).permute(0, 2, 3, 4, 1)
